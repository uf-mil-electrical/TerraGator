...
static int __init my_init(void) {
	printk("Initiating gpio interrupt...\n");
    	...
    	kobj_ref = kobject_create_and_add("sonar_2_time", kernel_kobj);
    	if (!kobj_ref)
	{
        return -ENOMEM;
	}
	...
	int gpio_stat = gpio_request(535, "rpi-gpio-23");
	if(gpio_stat == -EBUSY)
	{
	printk("ERROR: GPIO 23 is not free. free it or reboot\n");
	return -1;
	}
	if(gpio_stat == -EINVAL)
	{
	...
	if(gpio_direction_input(535))
	{
	printk("ERROR: GPIO 23 cannot be set as input\n");
	gpio_free(535);
	return -1;
	}
	//done setting up pin 13
	//setting up interrupt now
	irq_number = gpio_to_irq(535);
